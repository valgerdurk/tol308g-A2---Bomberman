...
function render(ctx) {
  ...
  if (eatKey(TOGGLE_CLEAR)) {
    g_doClear = !g_doClear;
    console.log('g_doClear = ' + g_doClear);
  }
  if (eatKey(TOGGLE_BOX)) {
    g_doBox = !g_doBox;
    console.log('g_doBox = ' + g_doBox);
  }
  if (eatKey(TOGGLE_UNDO_BOX)) {
    g_undoBox = !g_undoBox;
    console.log('g_undoBox = ' + g_undoBox);
  }
  if (eatKey(TOGGLE_FLIPFLOP)) {
    g_doFlipFlop = !g_doFlipFlop;
    console.log('g_doFlipFlop = ' + g_doFlipFlop);
  }
  if (eatKey(TOGGLE_RENDER)) {
    g_doRender = !g_doRender;
    console.log('g_doRender = ' + g_doRender);
  }
